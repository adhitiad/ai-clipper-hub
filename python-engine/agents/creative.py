...
def generate_creative_brief(insight: str, niche: str, target_audience: str):
    logger.info(f"Creative Agent: Meracik hook untuk {target_audience}...")
    base_personas = {
    ...
    llm = ChatGroq(model="gpt-oss-120b", temperature=0.8, api_key=config.GROQ_API_KEY)
    ...
    result = (prompt | llm).invoke({"insight": insight, "niche": niche, "target_audience": target_audience, "persona_instruction": persona_instruction}).content
    return result
